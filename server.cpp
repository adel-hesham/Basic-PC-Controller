...
makesocket::makesocket(int port)
{
    try
    {
...
makesocket::makesocket(int port,std::string address)
{
    server_socket=create_server_socket();
    sockaddr_in server_add;
    server_add.sin_family=AF_INET;
    server_add.sin_port=htons(port);
    inet_pton(AF_INET,address.c_str() , &server_add.sin_addr);
    if(bind(server_socket,(struct sockaddr*)&server_add, sizeof(server_add))==-1)
    {
        perror("bind()");
        close(server_socket); 
...
bool makesocket::close_program()
{
    ...
    makesocket::capture<<"Closing..."<<std::endl;
    makesocket::sendout(makesocket::capture.str());
    return false;

}
const int makesocket::create_server_socket()
{
